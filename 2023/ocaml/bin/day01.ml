...
let part_1 lines = 
  ...
  | [] -> 0
  | x :: xs -> 
    part_1_find_digit (String.to_list x) * 10 + part_1_find_digit (String.to_list_rev x) + (aux xs)
  in
  aux lines
;;
...
let part_2_find_digit line rev=
  ...
  | [] -> 0
  | x :: xs -> contains_digit str |> function
    | Some value -> value
    | None -> Char.is_digit x |> function
      | true -> Char.to_int x - Char.to_int '0'
      | false -> 
        aux
          (match rev with
...
let part_2 lines =
  ...
  | [] -> 0
  | x :: xs ->
    part_2_find_digit (String.to_list x) false * 10 + part_2_find_digit (String.to_list_rev x) true + (aux xs)
  in
  aux lines
;;
