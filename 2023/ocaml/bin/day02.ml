...
let parse_draws line =
  List.map (String.split ~on:';' (List.nth_exn (String.split ~on:':' line) 1)) ~f:(fun line ->
    List.fold ~init:(Map.empty (module String)) (String.split ~on:',' line) ~f:(fun acc pair ->
      let split = String.split ~on:' ' (String.strip pair) in
      Map.set acc ~key:(List.nth_exn split 1) ~data:(int_of_string (List.nth_exn split 0))
    )
  )

let limits = Map.of_alist_exn (module String) [
  ("red", 12);
  ("green", 13);
  ("blue", 14);
]
...
let minimum_bag draws =
  ...
  List.fold ["red"; "green"; "blue"] ~init:1 ~f:(fun acc key ->
    let maximum = (List.fold draws ~init:0 ~f:(fun acc draw ->
